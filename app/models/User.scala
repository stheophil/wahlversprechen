...
object User {
  ...
  private def makeSalt = (for (i <- 1 to 20) yield util.Random.nextPrintableChar).mkString
  ...
  def edit(id: Long, email: String, name: String, password: Option[String] = None, role: Option[Role] = None) {
    DB.withConnection {
      implicit c =>
