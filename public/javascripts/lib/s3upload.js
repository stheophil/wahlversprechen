      ...
      if (!xhr) {
        ...
        xhr.onload = function() {
          if (xhr.status === 200) {
            ...
            var version = xhr.getResponseHeader("x-amz-version-id");
            var public_url_with_version = public_url;
            if(version!==null) {
              if(public_url_with_version.indexOf("?")===-1) {
                public_url_with_version += "?";
              } else {
                public_url_with_version += "&";
              }
              public_url_with_version += "versionId=" + version;
            }
            return this_s3upload.onFinishS3Put(public_url_with_version);
          } else {
